...
naturalGasPrices = readmatrix('Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'D2:D48');
elecPrices = readmatrix('Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'E2:E48')/1000;
PPP = readmatrix('Prices regionalization.xlsx', 'Sheet', 'PPP', 'Range', 'J14:K17');
PPP = PPP(:,2)./PPP(:,1); % equalize with respect to Europe
...
ammoniaPricesWithoutElec = zeros(length(naturalGasPrices), 1);
j = 1;
...
writematrix(prices, 'Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'F2')
...
gAmmoniaPriceWithoutHElec = zeros(length(naturalGasPrices), 1);
...
windLCOHV = readmatrix('Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'N2:N5')'*1000; 
solarLCOHV = readmatrix('Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'O2:O5')'*1000; 

windLCOH = zeros(length(elecPrices), 1);
solarLCOH = zeros(length(elecPrices), 1);

for i = 1:length(elecPrices)
    if i > 0 && i <= 12
        windLCOH(i,:) = windLCOHV(1);
        solarLCOH(i,:) = solarLCOHV(1);
    elseif i > 12 && i <= 24
        windLCOH(i,:) = windLCOHV(2);
        solarLCOH(i,:) = solarLCOHV(2);
    elseif i > 24 && i <= 36
        windLCOH(i,:) = windLCOHV(3);
        solarLCOH(i,:) = solarLCOHV(3);
    else
        windLCOH(i,:) = windLCOHV(4);
        solarLCOH(i,:) = solarLCOHV(4);
    end
end

gWindAmmoniaPrices = gAmmoniaPriceWithoutH + windLCOH .* hydrogenReq ./ gAmmoniaProduced;
gSolarAmmoniaPrices = gAmmoniaPriceWithoutH + solarLCOH .* hydrogenReq ./ gAmmoniaProduced;

writematrix(gWindAmmoniaPrices, 'Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'G2')
writematrix(gSolarAmmoniaPrices, 'Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'H2')
...
methanolPricesWithoutElec = zeros(length(naturalGasPrices), 1);
j = 1;
for i = 1:length(naturalGasPrices)
    if i > 0 && i <= 12
        j = 1;
...
writematrix(prices, 'Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'I2')
...
gMethanolPriceWithoutHElec = zeros(length(naturalGasPrices), 1);
% green ammonia from grid electricity
for i = 1:length(elecPrices)
    if i > 0 && i <= 12
        j = 1;
...
gWindMethanolPrices = gMethanolPriceWithoutH + windLCOH .* H2Req;
gSolarMehtanolPrices = gMethanolPriceWithoutH + solarLCOH .* H2Req;

writematrix(gWindMethanolPrices, 'Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'J2')
writematrix(gSolarMehtanolPrices, 'Prices regionalization.xlsx', 'Sheet', 'US', 'Range', 'K2')